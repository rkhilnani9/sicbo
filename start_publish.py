...
common_image_paths = glob.glob('/Users/rkhilnan/Desktop/sicbo/content/common_segment/*.jpg')
...
for combo in all_combinations:
    paths = glob.glob(f'frames/*_{combo}/seg4/*.jpg')
    paths = sorted(paths, key=lambda x: int(
        x.split('/')[-1].split('.')[0]))
...
while True:
    ...
    try:
        ...
        for ix, impath in enumerate(common_image_paths):
            ...
            time.sleep(1/38)
            cv2.putText(im, f'Common: {k}', (10, 100),
        ...
        dice_numbers = open("dragon_tiger_cards.txt", "r").read()
        dice1 = dice_numbers.split(',')[0]
        dice2 = dice_numbers.split(',')[1]
        dice3 = dice_numbers.split(',')[2]
        ...
        for ix, impath in enumerate(n_paths):
            ...
            time.sleep(1/38)
            cv2.putText(
                im, str(k), (10, 100), font, 3, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(im, str(m), (10, 450), font,
